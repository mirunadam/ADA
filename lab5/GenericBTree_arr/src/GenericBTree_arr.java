...
class GenericBTree<K extends Comparable<K>, V> implements BTreemethods<K, V> {
    ...
    class BTreeNode{
        ...
        K key[] = (K[]) new Comparable[2 * T - 1];   //maximum 2T-1 keys
        BTreeNode child[] = (BTreeNode[]) new GenericBTree.BTreeNode[2 * T]; // maximum 2T children
    ...
    private boolean contains(BTreeNode node, K key) {
        ...
        // If we reach a leaf node, key is not present
        if (node.leaf) {
            return false;
        }
    ...
    private int level(BTreeNode node, K key, int currentLevel) {
        ...
        return level(node.child[i], key, currentLevel + 1);
    }
    ...
    final private void insertNonfullStart(BTreeNode x, K k) {
        ...
        if (x.leaf) {
            ...
            if (tmp.n == 2 * T - 1) {
                ...
                split(x, i, tmp);
                if (k.compareTo(x.key[i])>0) { // determines in which split half we insert
                    i++;
                }
            }
