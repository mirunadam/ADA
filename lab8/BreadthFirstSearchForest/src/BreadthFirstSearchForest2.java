...
class SimpleEdge2 {
    ...
    public SimpleEdge2(int either, int other) {
        this.either = either;
        this.other = other;
    }
 ...
 */
interface ISimpleGraph2 {
    int getNrVertices();
    int getNrEdges();
    void addUndirectedEdge(int either, int other);
    Iterable<Integer> nodesAdjacentTo(int node);
    Iterable<SimpleEdge2> edgesAdjacentTo(int node);
    Iterable<SimpleEdge2> allEdges();
 ...
 */
class SimpleGraphMatrix2 implements ISimpleGraph2 {
     ...
     */
    public SimpleGraphMatrix2(int V) {
        this.V = V;
        E = 0;
        g = new int[V][V];
     ...
     * @throws IOException
     */
    public SimpleGraphMatrix2(String file) throws IOException {
        initFromFile(file);
    }
     ...
     */
    public Iterable<SimpleEdge2> allEdges() {
        Set<SimpleEdge2> edgeSet = new HashSet<SimpleEdge2>();
        for (int node1 = 0; node1 < V; node1++)
            for (int node2 = node1; node2 < V; node2++)
                if (g[node1][node2] == 1) { // in undirected graphs make sure to
                    ...
                    SimpleEdge2 ed = new SimpleEdge2(node1, node2);
                    edgeSet.add(ed);
                }
        return edgeSet;
    }
     ...
     */
    public Iterable<SimpleEdge2> edgesAdjacentTo(int node) {
        Set<SimpleEdge2> edgeSet = new HashSet<SimpleEdge2>();
        for (int i = 0; i < V; i++)
            if (g[node][i] == 1) {
                SimpleEdge2 ed = new SimpleEdge2(node, i);
                edgeSet.add(ed);
            }
        return edgeSet;
    }
...
class BreadthFirstSearch2 {
     ...
     */
    public BreadthFirstSearch2(ISimpleGraph2 G, int s) {
    ...
    private void bfs(ISimpleGraph2 G, int source) {
        int V = G.getNrVertices();
    ...
    public void bfsForest() {
        ...
        for (int i = 0; i < V; i++) { //start from 0
            if (color[i] == BreadthFirstSearch2.Color.WHITE) { //when we find an undiscovered node it becames root
...
class BreadthFirstSearchForest2 {

    public static void main (String[]args){
        try {
            SimpleGraphMatrix2 graph = new SimpleGraphMatrix2("src/demo1.txt");
            System.out.println("Starting from root of the tree 0");
            BreadthFirstSearch2 bfsForest = new BreadthFirstSearch2(graph, 0);
            bfsForest.bfsForest();
        } catch (IOException e) {
            e.printStackTrace();
        }
